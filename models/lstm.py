...
class WaveLSTM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1 = nn.Conv1d(
            in_channels=hidden_dim * 2, out_channels=30, kernel_size=3,padding =1
        )
        self.bn1 = torch.nn.BatchNorm1d(30)
        
        self.conv2 = nn.Conv1d(
            in_channels= 30 , out_channels= 10, kernel_size= 3, padding=1
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        feat = self.conv1(feat)  # (B, C, T)
        feat = F.relu(self.bn1(feat))  # (B, C, T)
...
if __name__ == "__main__":
    ...
    model = WaveLSTM(num_frames=20, input_len=16000, hidden_dim=50, out_dim=1).to(device)
    x = torch.Tensor(np.random.rand(8, 16000))
    x = x.to(device)
    y = model(x)
    print(y.shape)
    print(y)
