...
from loss.MultiLoss import CustomLoss
...
class ModelTrainer(Trainer):
    ...
    def train(
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for _, (batch_x,batch_x_R, _, _, batch_y,_) in enumerate(train_loader):
                ...
                if channel:
                    batch_x_R = batch_x_R.to(self.device)
                    if model_classname == 'WaveLSTM':
                        batch_x = torch.cat((batch_x,batch_x_R),dim =1 )
                    elif model_classname == "SimpleLSTM":
                        batch_x = torch.cat((batch_x,batch_x_R), dim=2)
                    else:
                        batch_x = torch.stack((batch_x,batch_x_R),dim =1)
                # forward
            ...
            for batch_x,batch_x_R, _, _, batch_y , _ in test_loader:
                # get actual batch size
                curr_batch_size = batch_x.size(0)
                num_total += curr_batch_size
                # make batch label y a vector
                ...
                # count number of correct predictions
                num_correct += (batch_pred == batch_y.int()).sum(dim=0).item()
                total_loss_val += batch_loss.item() * curr_batch_size
            total_loss_val /= num_total
            ...
            LOGGER.info(
                f"[{epoch:03d}]: loss: {round(total_loss, 4)} - train acc: {round(train_acc, 4)} - test acc: {round(test_acc, 4)} - test eer : {round(test_eer, 4)} -val loss: {round(total_loss_val ,4)} "#- best threshold : {round(best_threshold, 4)}"
            )
            if test_acc >= best_acc:
                if total_loss_val < best_loss:
                    best_loss = total_loss_val
                    best_acc = test_acc
                    LOGGER.info(f"Best Test Accuracy: {round(best_acc, 4)} , Best Val Loss : {round(total_loss_val, 4)}")
    ...
    def eval(
        ...
        for batch_x,batch_x_R, _, path, batch_y,_ in test_loader:
            # get actual batch size
            curr_batch_size = batch_x.size(0)
            num_total += curr_batch_size
            # get batch input x
            ...
            # get binary prediction {0, 1}
            batch_pred = (torch.sigmoid(batch_out) + 0.5).int()
            pred_sigmoid = torch.sigmoid(batch_out)
            y_pred.append(batch_pred.clone().detach().cpu())
            # count number of correct predictions
            num_correct += (batch_pred == batch_y.int()).sum(dim=0).item()
            for i in range(len(path)):
                results[str(path[i])] = {'pred': round(float(pred_sigmoid[i]), 4), 'label': int(batch_y[i])}
